...
function down_release {
	local down_url="https://ghproxy.com/https://github.com/tModLoader/tModLoader/releases/download/$1/tModLoader.zip"
	if [[ -v version ]]; then
		set -- "$version"
	fi
...
function copy_worlds {
	for file in *.wld; do
		...
		mkdir -p ~/.local/share/Terraria/tModLoader-1.4.3/Worlds/ && cp $file $twld $_  
	done
}
...
function install_mods {
	...
	if [[ -v mods_path ]]; then
		...
		mods_path=$XDG_DATA_HOME/Terraria/tModLoader-1.4.3/Mods
		echo "Moving mods to $mods_path"
	else
		mods_path=~/.local/share/Terraria/tModLoader-1.4.3/Mods
		echo "Moving mods to $mods_path"
	fi
